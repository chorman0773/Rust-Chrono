...
/// There are no Limitations to Period, though it is recommended that it be a specialization of crate::ratio::Ratio (and therefore implement the Period trait)
///
/// Note:
...
unsafe impl<Repr: Zeroable, Period> Zeroable for Duration<Repr, Period> {}
unsafe impl<Repr: Pod, Period: 'static> Pod for Duration<Repr, Period> {}
...
impl<R1: TryInto<R2>, R2, P: Period> DurationTryInto<R2> for Duration<R1, P> {
    ...
    fn try_into(self) -> Result<Duration<R2, P>, Self::Error> {
        self.value.try_into().map(|value| Duration {
            value,
            _phantom: PhantomData,
        })
    }
...
where
    R1: Mul<i64>,
    <R1 as Mul<i64>>::Output: Div<i64, Output = R1::Output>,
    R1::Output: TryInto<R2>,
{
    ...
    fn duration_cast(
        self,
    ) -> Result<Duration<R2, P2>, <Self as DurationCast<Duration<R2, P2>>>::Error> {
        let (num, denom) = (
            RatioDivide::<P1, P2>::NUMERATOR,
            RatioDivide::<P1, P2>::DENOMINATOR,
        );
...
unsafe impl<R1: Step, P1> Step for Duration<R1, P1> {
    ...
    fn forward_checked(start: Self, count: usize) -> Option<Self> {
        Step::forward_checked(start.value, count).map(|value| Duration::new(value))
    }

    fn backward_checked(start: Self, count: usize) -> Option<Self> {
        Step::backward_checked(start.value, count).map(|value| Duration::new(value))
    }
}
