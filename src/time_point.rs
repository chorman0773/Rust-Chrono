...
unsafe impl<Duration: Zeroable, Clock> Zeroable for TimePoint<Clock, Duration> {}
unsafe impl<Duration: Pod, Clock: 'static> Pod for TimePoint<Clock, Duration> {}
...
impl<_Clock: Clock, _Duration: IsDuration> TimePoint<_Clock, _Duration> {
    pub fn into<R2>(self) -> TimePoint<_Clock, Duration<R2, _Duration::Period>>
    where
        _Duration: DurationInto<R2>,
    {
        TimePoint {
    ...
    pub fn try_into<R2>(
        ...
        TimePoint<_Clock, Duration<R2, _Duration::Period>>,
        <_Duration as DurationTryInto<R2>>::Error,
    >
    where
        _Duration: DurationTryInto<R2>,
    {
        Ok(TimePoint {
...
pub fn time_point_cast<D1: DurationCast<D2>, D2: IsDuration, _Clock: Clock>(
    _tp: TimePoint<_Clock, D1>,
) -> Result<TimePoint<_Clock, D2>, <D1 as DurationCast<D2>>::Error> {
    _tp.value.duration_cast().map(|value| TimePoint {
        value,
        _phantom: PhantomData,
    })
}
...
impl<D1: PartialEq<D2>, D2, _Clock: Clock> PartialEq<TimePoint<_Clock, D2>>
    for TimePoint<_Clock, D1>
{
...
impl<D1: PartialOrd<D2>, D2, _Clock: Clock> PartialOrd<TimePoint<_Clock, D2>>
    for TimePoint<_Clock, D1>
{
...
impl<D1: Default, _Clock> Default for TimePoint<_Clock, D1> {
    fn default() -> Self {
        Self {
...
impl<D1: Clone, _Clock> Clone for TimePoint<_Clock, D1> {
    fn clone(&self) -> Self {
        Self {
...
impl<D1: Copy, _Clock> Copy for TimePoint<_Clock, D1> {}
